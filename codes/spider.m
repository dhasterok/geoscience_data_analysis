function [p,ndata] = spider(data,normref,varargin);
...
[p,ndata] = spiderplot(data,refchem,ellist,Q);
...
if ischar(list)
    switch list
        case 'all'
            ellist = {'Li', 'K', 'Rb', 'Cs', 'Tl', 'Pb', ...
                'Ba', 'Th', 'U', 'Nb', 'La', 'Ce', 'Sr', ...
                'Pr', 'Nd', 'Zr', 'Sm', 'Eu', 'Gd', 'Tb', ...
                'Ti', 'Dy', 'Y', 'Ho', 'Er', 'Tm', 'Yb', ...
                'Lu', 'Sc', 'V', 'Zn', 'Cu', 'Cr'};
        case 'ree'
...
for i = 1:length(ellist)
    ...
    elseif strcmp(field,'cr_ppm')
        data.cr_ppm(ind) = 2/3*molecularwt('Cr')/molecularwt('Cr2O3')*data.cr2o3*10000;
