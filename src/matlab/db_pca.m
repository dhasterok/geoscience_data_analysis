function [z, coeff, score, latent, tsquared, explained, mu, scale] = pca_gdb(data,varargin)
% pca_gdb - Principal component analysis applied to geochemical data.
%
%   [z, coeff, score, latent, tsquared, explained, mu, scale] =
%   pca_gdb(data)
...
addParameter(p,'Fields',pcafields,@iscell);
addParameter(p,'CenteringMethod','none',@ischar);
addParameter(p,'ClassName','rock_type',@ischar);
addParameter(p,'ClassTypes','',@iscell)
...
if isempty(p.Results.ClassTypes)
    classtypes = unique(data.clasname);
else
...
for i = 1:length(classtypes)
    %subplot(131);
    hold on;
    pcahist(score(:,1:2),coeff(:,1:2),strcmp(data.(classname),classtypes{i}),vl,'PC 1','PC 2',classtypes{i});
    if i == 1
        plot([zeros([1 length(explained)]); coeff(:,1)'],[zeros([1 length(explained)]); coeff(:,2)'],'k-');
        for j = 1:length(explained)
            if length(explained) == length(pcafields)
                text(coeff(j,1),coeff(j,2),pcafields{j});
            else
