...
%
%    DELTA = SPHDIST(LON,LAT,LON0,LAT0) computes the angle, DELTA,
%    between the set of points given by (LON,LAT) and a reference
%    (LON0,LAT0) on a sphere.  The angle DELTA is given in degrees.
%
%    DELTA = SPHDIST(LON,LAT,LON0,LAT0,H) computes the signed angle,
...
delta = acos(dotprod(p0,p)./(normv(p0)*normv(p)));
